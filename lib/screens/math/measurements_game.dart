              ...
              GameSecurityOverlay(
                ...
                Positioned.fill(
                  child: LeaveWarningOverlay(
                    ...
                    // `_saveGate.isSaving` used to be the only guard here, and
                    // it is false at the moment the overlay appears - so both
                    // buttons stayed live through the whole exit.
                    isBusy: _isLeavingScreen || _saveGate.isSaving,
                    busyText: 'Leaving...',
                  ),
                ),
            ],
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final width = desktop
            ? panelWidth.clamp(280.0, 860.0).toDouble()
            : panelWidth.clamp(280.0, 720.0).toDouble();
        final height = desktop
            ? panelHeight.clamp(220.0, 440.0).toDouble()
            : panelHeight.clamp(220.0, 380.0).toDouble();
        final padding = desktop
            ? (width * 0.013).clamp(6.0, 10.0).toDouble()
            : (width * 0.020).clamp(6.0, 12.0).toDouble();
        ...
        final keyWidth = desktop
            ? ((contentWidth - spacing * 3) / 4).clamp(42.0, 190.0).toDouble()
            : ((contentWidth - spacing * 3) / 4).clamp(42.0, 150.0).toDouble();
        ...
        final fontSize = desktop
            ? (min(keyWidth, keyHeight) * 0.5).clamp(20.0, 38.0).toDouble()
            : (min(keyWidth, keyHeight) * 0.44).clamp(15.0, 31.0).toDouble();
        final radius = desktop
            ? (min(keyWidth, keyHeight) * 0.14).clamp(6.0, 10.0).toDouble()
            : (min(keyWidth, keyHeight) * 0.24).clamp(8.0, 16.0).toDouble();
                        ...
                        SizedBox(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              _button('0', keyWidth, keyHeight, fontSize, radius),
                        ...
                        Row(
                          ...
                          children: [
                            for (var col = 0; col < rows[row].length; col++) ...[
                              _button(rows[row][col], keyWidth, keyHeight, fontSize, radius),
