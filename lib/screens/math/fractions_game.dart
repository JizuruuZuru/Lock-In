...
class _FractionAnswerPad extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final desktop = isDesktopLayout(panelWidth);
    final width = desktop
        ? panelWidth.clamp(280.0, 900.0).toDouble()
        : panelWidth.clamp(280.0, 760.0).toDouble();
    final height = desktop
        ? panelHeight.clamp(220.0, 420.0).toDouble()
        : panelHeight.clamp(220.0, 360.0).toDouble();
    final padding = desktop
        ? (width * 0.014).clamp(6.0, 10.0).toDouble()
        : (width * 0.022).clamp(6.0, 12.0).toDouble();
    final spacing = desktop
        ? (width * 0.008).clamp(3.0, 6.0).toDouble()
        : (width * 0.012).clamp(3.0, 7.0).toDouble();
    ...
    final keyWidth = desktop
        ? ((contentWidth - spacing * 3) / 4).clamp(44.0, 190.0).toDouble()
        : ((contentWidth - spacing * 3) / 4).clamp(44.0, 152.0).toDouble();
    final keyHeight = desktop
        ? ((height - padding * 2 - spacing * 3 - actionGap) / rows.length)
            .clamp(28.0, 78.0)
            .toDouble()
        : ((height - padding * 2 - spacing * 3 - actionGap) / rows.length)
            .clamp(28.0, 62.0)
    ...
    final fontSize = desktop
        ? (min(keyWidth, keyHeight) * 0.5).clamp(20.0, 36.0).toDouble()
        : (min(keyWidth, keyHeight) * 0.44).clamp(15.0, 30.0).toDouble();
    final radius = desktop
        ? (min(keyWidth, keyHeight) * 0.14).clamp(6.0, 10.0).toDouble()
        : (min(keyWidth, keyHeight) * 0.26).clamp(8.0, 16.0).toDouble();
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(
            desktop
                ? (width * 0.024).clamp(10.0, 16.0).toDouble()
                : (width * 0.04).clamp(16.0, 28.0).toDouble(),
          ),
                  ...
                  if (r == rows.length - 1)
                    SizedBox(
                      ...
                      child: Row(
                        ...
                        children: [
                          _button('Delete', keyWidth * 1.35, keyHeight, fontSize, radius),
                          _button('BACK', keyWidth, keyHeight, fontSize, radius),
                          _button('Enter', keyWidth * 1.35, keyHeight, fontSize, radius),
                        ],
                      ),
                    )
                  else
                    Row(
                      ...
                      children: [
                        for (var i = 0; i < rows[r].length; i++) ...[
                          _button(rows[r][i], keyWidth, keyHeight, fontSize, radius),
  ...
  Widget _button(String label, double width, double height, double fontSize, double radius) {
